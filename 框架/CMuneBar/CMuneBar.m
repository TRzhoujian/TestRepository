...
-(instancetype)initWithItems:(NSArray *)itemsImages size:(CGSize)size type:(MuneBarType)type{
    ...
    if (self) {
        ...
        self.isShow = NO;
        self.type = type;
        self.frame = CGRectMake(ZN_SCREEN_WIDTH - size.width - 10, ZN_SCREEN_HEIGHT/30, size.width, size.height);
        [self addSubviews];
    }
    return self;
}
 ...
 */
-(void)addSubviews{
    MyButton *mainButton = [MyButton buttonWithType:UIButtonTypeCustom];
...
//    [mainButton setBackgroundColor:RGB(178, 178, 178)];
    [mainButton setBackgroundColor:[UIColor whiteColor]];
//    [mainButton setTitle:@"Mune" forState:UIControlStateNormal];
    [mainButton setImage:[UIImage imageNamed:@"icon_tools"] forState:UIControlStateNormal];
    [mainButton setImage:[UIImage imageNamed:@"icon_up"] forState:UIControlStateSelected];
    mainButton.imageView.contentMode = UIViewContentModeScaleAspectFit;
